...
def get_training_testing_sm_datasets(split_val=0.8, random_seed=None):
    ...
    sample_dataset, info = tfds.load('santa_maria_dataset/body', with_info=True, data_dir = data_dir)
    ...
    def generate_data(data_type):
        ...
        for patient_id in patient_list:            
            for data in patient_data[patient_id]:
                ...
                _, data_roi = utils.get_roi(img_exam, mask_exam, padding = padding)                           
                data_roi = np.resize(data_roi, (size, size))
                yield data_roi, data['label']

    # Create a TensorFlow Dataset from the generator
    training_dataset = tf.data.Dataset.from_generator(lambda : generate_data('training'), 
                                                      output_signature=(
                                                          tf.TensorSpec(shape=(size, size), dtype=tf.float32, name="image"),
